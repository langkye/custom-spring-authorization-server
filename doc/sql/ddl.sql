...
) comment 'oauth2_authorization_consent';
...
) comment 'oauth2_registered_client';
...
(
    id                      bigint       not null auto_increment  comment '数据标识'                          primary key,
...
create index login_types on system_user (login_types);
...
(
    ...
    constraint system_user_login_system_user_id_fk foreign key (system_user_id) references system_user (id)
) comment '用户登录信息';
...
(
    ...
    constraint fk_users_roles_role_id_mooc_roles_id foreign key (role_id) references system_role (id),
    constraint fk_users_roles_user_id_mooc_users_id foreign key (user_id) references system_user (id)
) comment '用户角色关联表';

create table oauth2_registered_key
(
    id                       bigint                              not null auto_increment comment '数据标识'   primary key,
    create_by_user_id        int                                 null                    comment '创建用户id',
    create_by_user_name      varchar(128)                        null                    comment '创建用户姓名',
    ...
    status                   int                                 null                    comment '数据状态',
    is_delete                int                                 null                    comment '是否删除',
    key_issued_at            timestamp default CURRENT_TIMESTAMP not null                comment '签发时间',
    key_expires_at           timestamp                           null                    comment '到期时间',
    key_id                   varchar(255)                        not null                comment '密钥唯一标识',
    algorithm                varchar(64)                         not null                comment '算法',
    private_key              text                                not null                comment '算法私钥',
    public_key               text                                null                    comment '算法公钥',
    constraint oauth2_registered_key_kid_uindex unique (key_id)
) comment 'oauth2_registered_key';
