 ...
 */
package sample.config.provider.consent.converter;
 ...
 */
public class CustomOAuth2AuthorizationConsentAuthenticationConverter implements AuthenticationConverter {
	...
	public Authentication convert(HttpServletRequest request) {
		...
		if (parameters.containsKey(OAuth2ParameterNames.SCOPE)) {
			List<String> strings = parameters.get(OAuth2ParameterNames.SCOPE);
			strings.forEach(it -> {
				if (StringUtils.hasText(it.trim())) {
					String[] split = it.trim().split(" ");
					for (String s : split) {
						if (Objects.nonNull(s) && !(Objects.equals("", s.trim()))) {
							scopes.add(s);
						}
					}
				}
		...
		return new CustomOAuth2AuthorizationConsentAuthenticationToken(authorizationUri, clientId, principal,
